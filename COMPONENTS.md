...
**Donut value arc is a gradient, not a flat stroke.** The value arc's `<circle>` is painted with `stroke={url(#gradientId)}` referencing an SVG `<linearGradient>` built from the same two-stop tone pairs as `ParticipationScale`'s active segment — `status-success-strong` → `status-success` for the success tone, `status-warning-subtle` → `status-warning` for the warning tone — rather than a single flat `stroke-status-success`/`stroke-status-warning`. This is a direct match to Figma's real gradient ring (previously implemented as a flat stroke as a placeholder) and makes the donut and the segmented scale bar read as one consistent color story for a given tone, since they now share the identical gradient stop pair. Because the ring's `<svg>` is mirrored/rotated (see above), the gradient itself is defined running local-right-to-left (`x1="100%" y1="50%"` → `x2="0%" y2="50%"`) rather than the more obvious top-to-bottom `0%→100%` — this is what cancels the mirror out so the gradient still reads top-to-bottom on screen instead of rotating along with the ring's sweep direction.
