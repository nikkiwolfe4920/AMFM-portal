...
- **The asset itself must be a plain rectangular photo, not a flattened export of the assembled Figma component**: the previously-committed `Featured Training.png` was an export of the whole "Featured Training" component, so it carried that component's own `rounded-2xl` + drop-shadow *as baked-in alpha* — its bottom ~24px (and a small arc at each corner) were partially/fully transparent in the file. Composited into `ElevatedCard`'s independently-rounded `background` layer, that transparency let the page's `bg-background` show through as a translucent band along the bottom edge, reading as if the photo didn't reach the card's full 512px height (`object-cover`/`fill` were filling the container correctly the whole time — the source pixels simply weren't opaque to their own edge). Fixed by re-cropping the file to its fully-opaque interior (a 28px inset clears the corner arcs; full opacity holds through row 511 of 536 before the original bottom fade began) and flattening to RGB — see `DESIGN.md`'s Known gaps for the pixel-level verification. If this asset is re-exported from Figma again, export the photo layer alone (or re-run the same alpha-edge crop) rather than a screenshot of the finished component.
