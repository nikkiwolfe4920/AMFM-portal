...
**Figma**: AMFM Portal file, "Featured Training" component (node `4194:25820`, superseding the previously-referenced node `2318:26997`; also placed as node `3926:27022` in the HeartChart Resources page composition), rendered on the `/heartchart-resources` page as the "Let's prepare for your HeartChart Weekend" banner
...
`text-nav-foreground` (first heading line — Figma's `text-primary-(900)` variable resolves to `#f7f7f7` on this fixed-dark surface, an exact match), `text-highlight-gold` (second heading line, `#e9c481` exact match), `text-nav-foreground-muted` (description, `#cecfd2` exact match), `text-display-lg`/`text-display-2xl` + `font-display`, `aspect-top-hero` (named non-scale utility for the card's Figma-exact `169:64` aspect ratio, `src/tokens/spacing.css`) paired with `min-h-88` (352px, Tailwind's dynamic spacing scale) so the copy column never gets clipped below the aspect ratio's crop-flip threshold. `Button variant="outline" size="default"` for the CTA — no new button styling, and the size is pinned so it does not drift if `Button`'s default changes later. `shadow-photo-pinstripe` (`src/tokens/shadows.css`) pairs the inner panel's `border-white/30` with a second dark inset ring — see Implementation rules below.
...
Not yet evidenced against a Figma mobile/tablet frame (the reference is a fixed desktop-width composition, same category of gap as `HeartChartSummary`/`PricingCard`). The heading column caps at `max-w-top-hero-copy` (544px) per Figma; the outer `ElevatedCard` is fluid-width and holds `aspect-top-hero` (`169:64`, matching the Figma frame's exact `1352×512` proportion — confirmed via `get_metadata` on nodes `4194:25820` and `3926:27019`/`3926:27022`), so the card scales with the page but never departs from Figma's proportion at any viewport. This replaced an earlier fixed `h-128` (512px) height: pinning height while width stayed fluid let the background photo's `object-cover` crop flip from cropping top/bottom (wide containers) to cropping left/right (containers narrower than the photo's own aspect ratio) once the viewport got narrow enough — clipping the HeartChart QR screen on the right instead of matching Figma's framing. Fixing the aspect ratio instead of the height keeps the crop mode — and the amount of headroom around the stage and QR screen — identical to Figma at every width. A `min-h-88` (352px) floor on the same element stops the card from shrinking, at very narrow content widths, below the point where the heading/description/CTA column overflows the card — the floor sits under the aspect ratio's own crop-flip threshold (~364px, at the ~961px content width where the photo's cover-fit axis would otherwise flip), so it never re-triggers the crop bug it's meant to guard against; it only stops the card getting shorter once the viewport is already narrower than that range.

### Implementation rules

- **Figma-exact aspect ratio, no white gap, image bleeds to the true outer edge**: previously `TopHero` filled only `ElevatedCard`'s inner bordered panel, leaving the outer shell's `bg-background` (white) padding gap visible as a solid border around the photo — Figma shows no such gap: the photo fills the *entire* outer rounded-2xl shape, and only a thin `border-white/30` "pinstripe" line (not a solid-color gap) marks the 8px inset where the inner panel sits. Fixed by passing the photo/scrim layer to `ElevatedCard`'s new `background` prop (see `ElevatedCard` above) instead of rendering it inside the inner panel — `background` is absolutely positioned against the *outer* shell's own box, which has no border (only padding), so its containing block is flush with the true outer edge and it bleeds under the padding gap; the inner panel's `border-white/30` then reads as a pinstripe drawn over continuous photo, not a margin. The card holds `aspect-top-hero` (`169:64`) rather than a fixed height so the shape stays proportional to Figma's `1352×512` frame at every viewport instead of a fixed height combined with a fluid width silently changing the background photo's `object-cover` crop axis (see Responsive behavior).
...
- **Pinstripe paired with a dark inset ring so it reads on all four sides, not just top/left/right**: Figma node `3926:27019`'s inner panel border is a single `rgba(255,255,255,0.3)` (`alpha-white-30`) hairline on all four sides — the same treatment `border-white/30` already implements. Verified by rendering the actual page and sampling pixels (not just reading the source): the browser does paint that 1px border-bottom, matching border-top exactly in width/color, at every tested viewport. The problem is legibility, not a missing element — the border sits over near-black stage lighting at the top (high contrast, clearly visible) but over the busy, lighter-toned audience photo at the bottom (a translucent-white 1px line all but disappears there), so the pinstripe reads as present on three sides and absent on the fourth even though the CSS is identical on all four. `shadow-photo-pinstripe` (`src/tokens/shadows.css`) adds a second inset ring one pixel further in, using the same dark shadow color as `shadow-button-inset`'s inner ring (`oklch(0.158 0.012 260.622 / 18%)`) rather than inventing a new one. Box-shadow paints under the real border, so the outer 0–1px band still shows the Figma-specified white hairline (border always paints on top); the additional 1–2px band shows only the dark ring, which is what stays visible over lighter photo content. Net effect: at least one of the two lines has sufficient contrast against any point along the perimeter, regardless of what the photo shows directly behind it — a duo-tone hairline instead of a single-tone one that only works over dark backgrounds. This is `TopHero`-specific (via `innerClassName`, not a change to `ElevatedCard`'s own default border) because it is the only current `ElevatedCard` caller that bleeds a photo under the inner panel; `ElevatedCard`'s default `border-border` case sits on a solid `bg-background`, where this contrast problem does not exist.
...
**Donut value arc sweeps counter-clockwise from 12 o'clock, not clockwise.** Confirmed against Figma's own sample states — node `1670:36549`'s 1% instance shows its sliver sitting just *left* of the top marker, and node `4255:30880`'s 58% instance has the value arc covering the left/bottom/lower-right of the ring with the gray track occupying the upper-right — the opposite chirality from a `<circle>` element's native stroke-dasharray direction (which is always clockwise starting at 3 o'clock in every browser, a fixed rendering-engine behavior no rotation alone can reverse). The ring's `<svg>` uses `rotate-90 -scale-x-100` (rotate 90° clockwise, then mirror horizontally) rather than the more obvious `-rotate-90`: the mirror is what actually reverses the sweep's chirality to counter-clockwise; the rotation alone only relocates the start point back to 12 o'clock afterward. A previous implementation used `-rotate-90` alone, which places the start point correctly but sweeps clockwise — a shape mismatch that's easy to miss at a glance for round percentages but is visibly wrong once compared against Figma's asymmetric sample states.

**Donut value arc is a gradient, not a flat stroke.** The value arc's `<circle>` is painted with `stroke={url(#gradientId)}` referencing an SVG `<linearGradient>` built from the same two-stop tone pairs as `ParticipationScale`'s active segment — `status-success` → `status-success-strong` for the success tone, `status-warning-subtle` → `status-warning` for the warning tone — rather than a single flat `stroke-status-success`/`stroke-status-warning`. Both tones now share the same light-at-the-start → dark-at-the-end ordering along the sweep (`status-success` previously ran the opposite way, `-strong` first, which put the darker stop at 12 o'clock instead of at the tail end of the arc — a mismatch caught by re-comparing node `4255:30880`'s 58% sample, which is lighter at the top and darkens toward the gray cutoff). This is a direct match to Figma's real gradient ring (previously implemented as a flat stroke as a placeholder) and makes the donut and the segmented scale bar read as one consistent color story for a given tone, since they now share the identical gradient stop pair. Because the ring's `<svg>` is mirrored/rotated (see above), the gradient itself is defined running local-right-to-left (`x1="100%" y1="50%"` → `x2="0%" y2="50%"`) rather than the more obvious top-to-bottom `0%→100%` — this is what cancels the mirror out so the gradient still reads top-to-bottom on screen instead of rotating along with the ring's sweep direction.

**Donut ring is a thick 24px stroke with flat (butt) caps, not a thin rounded-cap stroke.** `strokeWidth={24}` on a `size={90}` ring (versus a previous `10`) matches the proportions of Figma's rendered ring — pixel-compared against node `4255:30880`'s 58% sample, whose hole diameter is roughly half the ring's outer diameter, far thicker than a thin progress-ring treatment. The value arc and track circle both omit `strokeLinecap`, defaulting to SVG's `butt` cap, matching Figma's sharp radial edges at both the 12 o'clock start point and the value/track boundary; `strokeLinecap="round"` (a previous placeholder) rounded both ends into a pill shape Figma's reference doesn't have, most visible at very low percentages where the value arc is a thin sliver.
...
- **Navigation** (scrollable): two `NavSection`s ("Your Church"/"Church" and "Ministry Tools"/"Tools" — see Implementation rules for why the heading text itself changes between states), each a subheading + a list of `NavItem`s (icon + label). An item marked `comingSoon` (e.g. "Date Night Kits") additionally renders a `NavComingSoonBadge` pill ("Coming Soon") after its label, once expanded — see States.
...
| Coming Soon (expanded only) — e.g. "Date Night Kits" | Figma-sourced (node `3727:25297`). Items with `comingSoon: true` render as a non-interactive `<div role="link" aria-disabled="true">` instead of a real `<Link>` — no `href`, not in the tab order — with a `NavComingSoonBadge` pill ("Coming Soon", `border-white/20` outline, `text-nav-foreground-muted`) after the label. The badge reveals/collapses with the rail exactly like the label itself (`max-w`/`opacity` transition, not a Figma-referenced behavior but matching the label's own established pattern). Hovering the row shows `cursor-not-allowed` (not Figma-sourced — a product decision per the task that introduced this state, signaling "not clickable" to sighted pointer users) and there is no `hover:bg-white/5` highlight, unlike ordinary items, since a hover highlight would misleadingly imply the row is interactive (see `DESIGN.md`'s "state reflects reality" principle). Never `active`, regardless of `activeHref`/route. |
...
Each nav item's data (`CHURCH_LINKS`/`TOOLS_LINKS`/`FOOTER_LINKS`, module-level, not a prop) is a `NavLinkItem`:
...
- `comingSoon` items render `role="link" aria-disabled="true"` on a plain `<div>` instead of a real `<Link>` — no `href`, so it isn't reachable by mouse/keyboard activation, and (having no `tabindex`) it isn't in the tab order either, matching how a native disabled control behaves. This is the WAI-ARIA "disabled link" pattern: `role="link"` tells assistive tech this was a link, `aria-disabled="true"` says it's currently unavailable. Both the label and the "Coming Soon" badge remain real, always-rendered text nodes (never `aria-hidden`, same "labels stay in the accessibility tree" philosophy as the rail's collapse/expand behavior above), so screen reader users encounter the full "Date Night Kits — Coming Soon" status when browsing the list, regardless of the rail's collapsed/expanded visual state.
...
- **`NavComingSoonBadge` is a small, non-exported sub-component local to `global-nav.tsx`, not a new `src/components/ui` primitive**: its outline-pill treatment (`border-white/20`, no fill) only ever applies inside the rail's fixed-dark chrome using nav-only tokens (`text-nav-foreground-muted`) — it is not the same shape as `StatusTag` (the codebase's other pill primitive, which is a filled bg/border/text pill for swappable light-surface contexts like table cells, with `success`/`error`/`warning` variants). Extending `StatusTag` with a nav-specific 4th variant would entangle a shared, theme-swappable component with an always-dark exception; per `CLAUDE.md`'s component governance, a new shared primitive is only justified once a second real, non-nav use case for this exact outline-pill treatment shows up — until then it stays local, matching `NavHeader`/`NavSection`/`NavAccountCard`'s existing precedent of file-local sub-components.
- **`NavItem` branches on `comingSoon` before deciding between a real `<Link>` and a non-interactive `<div>`**, rather than rendering a `<Link>` with a disabled-looking style: a `comingSoon` item has no real destination to navigate to yet (same category of gap as every other placeholder route), so keeping it a real, focusable, clickable `<Link>` would be misleading regardless of visual treatment — see Accessibility requirements above.
- **Coming-soon row spacing mirrors the Figma reference's own nested grouping exactly** (icon + label as one `gap-2` [8px] flex group, then a `gap-3` [12px] gap before the badge — node `3727:25297`'s "Text and icon" (`gap-md`/8px) and "Content" (`gap-lg`/12px) frames), rather than reusing the ordinary `NavItem`'s flat `gap-3` list of children (icon, label) directly: adding the ~96px-wide badge onto that flat 12px-gap layout would overflow the rail's expanded content width by a few pixels, since Figma's own frame only fits exactly at 8px between icon and label. The ordinary (non-`comingSoon`) `NavItem` keeps its pre-existing flat `gap-3` layout unchanged — this nested grouping is scoped to the `comingSoon` branch only.
- **No hover background on `comingSoon` rows**: ordinary items get `hover:bg-white/5`; `comingSoon` items intentionally omit it (only `cursor-not-allowed` signals hover) so the row never looks interactive — see States.

### Visual examples

Rendered live in the Marriage Champions shell (`/marriage-champions`, `/marriage-champions-empty`), on `/heartchart-resources`, and as a single inspectable instance (collapsed by default, non-`overlay`) at `/design-system/components#globalnav` — hover it (or Tab into it) to see the expanded state, and click the avatar/name to see the account menu. Previously `/design-system` rendered two side-by-side instances (one forced open via `defaultOpen`) to show both rail states at once; consolidated to one instance per the task that introduced hover-to-expand, since hovering the single instance now demonstrates both states directly. To see the overlay-not-push and pinned-open (≥1600px) behaviors live, view `/marriage-champions` or `/heartchart-resources` directly rather than the gallery instance, which intentionally stays non-`overlay`. The "Coming Soon" state is demoed on "Date Night Kits" (Ministry Tools section) in every one of these instances, since `comingSoon` is set on the shared `TOOLS_LINKS` data, not a gallery-only override — expand the rail and hover that row to see the badge and the `cursor-not-allowed` affordance.
...
Outer elevated shell (`ElevatedCard`, same nested outer-shadow-shell/inner-bordered-panel shape as `HeartChartSummary`) → WeDo wordmark logo (top-left) → optional "Next Pulse in {label}" countdown text (top-right) → a two-column row: **left column** — big stat number ("363 Couples", in the WeDo brand red), supporting sentence ("Active in the app today"), and the couple illustration (`public/We-do.png`, rendered wider-than-tall via `aspect-wedo-illustration w-full max-w-wedo-illustration`) stacked directly beneath it; **right column** — a `flex-col` stack containing `PointerCallout` (see below), stretched to fill the remaining width, with an uppercase "Most of your couples say..." label above the pull-quote (plain text, no surrounding quote punctuation of its own; optional highlighted phrase in the WeDo brand red), which itself has a large low-opacity serif `&ldquo;` glyph watermarked directly in front of its own first word (see Design tokens used) — not behind the label — plus a "Source: {quoteSource}" attribution line, and directly beneath the callout, the action row: two `Button` instances ("See Results", "Share Your Code"), right-aligned (`justify-end`) so their combined right edge lines up with `PointerCallout`'s own right edge.
...
Corrected a third time in the Figma-to-code cleanup pass: the decorative quotation mark was a small solid red Lucide `Quote` icon, and the couple illustration was forced to a square `size-[186px]` — both were placeholders pending closer verification. Figma's real pull-quote mark is a large, low-opacity serif glyph, and the illustration's real proportion is wider-than-tall (~223×156), not square — see Design tokens used and Implementation rules below for the corrected treatment of each.

Corrected a fourth time after a direct Figma MCP re-pull of node `4255:30880`: the decorative quotation mark was anchored to the *whole* `PointerCallout` box (`-top-3 -left-1` off the box's own top-left corner), which placed it directly on top of the "Most of your couples say..." label instead of the pull-quote text — Figma's `_Summary Data` instance positions the glyph relative to the quote paragraph itself (`top`/`-translate-y-full` off that text block, not the callout), so it reads as a watermark behind the quote's opening line, sitting below the label rather than obscuring it.

Corrected a fifth time after re-checking node `4255:30880`'s own reference code: the pull-quote text was still being wrapped in literal `&ldquo;`/`&rdquo;` characters, which Figma's source does not do — the design conveys "this is a quote" via the "Most of your couples say..." label and the decorative glyph alone, and renders the quote itself as plain text. The glyph's own offset (`-top-8 -left-2` off a wrapper around just the `<p>`) also wasn't tall enough to reach the paragraph's first line: the glyph's ink sits near the *top* of its 96px line box, so at that offset it visually landed beside the label above, not beside "When". Figma's own text node also opens with several literal leading spaces (reserving room for the glyph on the first line only) rather than any CSS indent. This pass drops the literal quote characters, moves the glyph to `-top-1.5 -left-1` (placing its ink at the paragraph's first line instead of the label), and replaces Figma's literal-leading-spaces trick with `indent-8` on the `<p>` — a first-line-only CSS text-indent that reserves the same gutter without embedding invisible whitespace characters in the quote copy itself.
...
The decorative quote mark uses `font-display text-8xl text-muted-foreground/75` — the existing display serif family at a large size and a low-opacity neutral tone, not a new token, absolutely positioned (`-top-1.5 -left-1`) relative to a `relative` wrapper around the pull-quote paragraph only (not the whole callout box) so its ink sits directly in front of the paragraph's first line ("When") rather than the "Most of your couples say..." label above it. The paragraph itself uses `indent-8` (a first-line-only `text-indent`, not a new token) to reserve a gutter for the glyph on its first line only — later wrapped lines start flush left, matching Figma. No `&ldquo;`/`&rdquo;` characters wrap the visible quote text; the glyph and the label above it are what convey "this is a quote," matching Figma's plain-text pull-quote.
...
- The large decorative quotation mark (a `<span>` rendering `&ldquo;`, not an image) is `aria-hidden` and `select-none` — it's a typographic flourish sitting in front of the pull-quote's opening line, restating that the text below is a quote, which the surrounding "Most of your couples say..." label already conveys to assistive tech. The quote text itself carries no surrounding quote punctuation of its own.
...
**Corrected**: the card's own internal stat/illustration column and pull-quote column used `flex flex-wrap`, which does not actually wrap a `flex-1 min-w-0` child — the browser keeps shrinking it instead of dropping it to a new line. Below `lg` on `/dashboard`, `WeDoCard` gets the full single-column width (still often under ~500px on phones), which collapsed the quote column to roughly one character per line instead of stacking below the stat/illustration column. Changed to `flex-col sm:flex-row` (the same stack-below/row-from-breakpoint pattern already used by `HeartChartLinkCard` and `SettingsAssetUpload`), so the two columns stack full-width below the `sm` breakpoint and sit side by side above it. The `PointerCallout` tail still points left (toward where the illustration sits in the row layout) even when stacked with the illustration now above it — an accepted trade-off given no confirmed mobile Figma reference exists for this component (see above).
...
- **Corrected**: the decorative quotation mark was previously a small, solid `text-wedo-brand` Lucide `Quote` icon (`-scale-x-100` to read as an opening quote) sitting above the label. Figma's real pull-quote mark is a large (`text-8xl`), low-opacity (`text-muted-foreground/75`) serif (`font-display`) `&ldquo;` glyph — a plain `<span>`, not an icon component, `aria-hidden` and `select-none` since it's purely decorative.
- **Corrected again** after a direct Figma MCP re-pull of node `4255:30880`: the glyph was absolutely positioned (`-top-3 -left-1`) off the *whole* `PointerCallout` box, which placed it on top of the "Most of your couples say..." label instead of the quote. It now lives inside a `relative` wrapper around just the pull-quote `<p>` (`-top-8 -left-2` off that wrapper), so it reads as a watermark behind the quote's opening line — matching both the Figma node and its reference screenshot — rather than obscuring the label above it.
...
- **Corrected**: `PointerCallout` was rendered here with an `overflow-hidden` class alongside `min-w-0`. Since the left-diagonal tail asset is deliberately positioned *outside* the box's own edge (`left-pointer-callout-tail`, -17px — see `PointerCallout`'s own entry) to hang off it, `overflow-hidden` clipped nearly the entire tail, making it invisible on both `/dashboard` and `/design-system/components#wedocard`. `min-w-0` alone (already present) is sufficient to let the quote text wrap inside the flex layout; `overflow-hidden` was dropped.
...
- Because `left-pointer-callout-tail` is a *negative* offset (-17px), the tail is designed to render outside the container's own box — callers must not add `overflow-hidden`/`overflow-clip` to `PointerCallout` (or a wrapping element) when using `"left-diagonal"`, or the tail is clipped away almost entirely. `WeDoCard` did exactly this for a time (paired with `min-w-0` to let its quote text wrap) — see that entry's Implementation rules for the fix.
...
- Corrected a positioning bug: both instances previously packed toward their own `lg:flex-1` column's *outer* edge (`"left"` flush against the page's left margin under `HeartChartSummary`, `"right"` flush against the page's right margin under `WeDoCard`), mirroring outward. Re-verified against node `4255:30880`'s two `"Pointer Call-out"` containers — the left one uses `items-end` and the right one uses `items-start` on their own column box, which both resolve to packing toward the *inner* seam between the two hero cards (the gap the two columns share), not the outer page edges. Fixed by adding `lg:justify-end` to the row (unconditional on `side`): combined with the existing `flex-row-reverse` on `"right"`, this packs `"left"` to the right of its column and `"right"` to the left of its column, so the pair reads as hugging the gap between the cards rather than fanning outward. Unscoped below `lg` so the existing mobile stacked layout (`flex-col gap-3`) is unaffected.
...
Icon + label header (`next/image` icon + "Age Groups") → vertical bar chart area with 4 horizontal gridlines behind the bars → one bar per category, with a bold value label stacked directly on top of that bar's own rendered height (not pinned to a fixed row at the top of the chart) and a category label below each bar.
...
- Value label (above each bar): `text-sm font-bold text-chart-participation-value` — a dedicated token (utility-brand-400, `#cf896b`) sampled from the same node; distinct from `--primary` (the app's button/accent terracotta), which is a different, more saturated brand shade. Rendered as the first child of a fixed-height (`h-32`), bottom-anchored (`flex flex-col items-center justify-end gap-0.5`) wrapper that also contains the bar div, so the label sits directly on the bar's own top edge and moves up/down with it — pixel-verified against node `4255:30880`'s `Full bar` → `percentage` (a `justify-end` flex column stacking the percentage text directly above the "Complete Activity" bar fill). It must not be rendered as a sibling above a separately-sized bar wrapper, since that pins the label to a fixed row regardless of bar height.
...
- **Corrected in a follow-up pass: the value label lives inside the same fixed-height, bottom-anchored column as the bar, not as a sibling rendered above a separate bar wrapper.** The earlier implementation put the `{value}%` label before the `h-32` bar wrapper in the outer `flex-col`, which pinned every label to the same fixed row at the top of the chart regardless of that bar's height — visibly wrong for short bars, whose labels floated far above their bar with a large gap. Node `4255:30880`'s `Full bar` node is itself the `justify-end` flex column (label + bar as its only two children, `overflow-clip` + a per-bar `pt-[...]` value that Figma renders as a fixed pixel clip encoding each bar's height); the fix moves the value label to be the bar div's own sibling inside that `h-32 flex-col items-center justify-end gap-0.5` wrapper so it always sits flush on top of the bar's current rendered height.
