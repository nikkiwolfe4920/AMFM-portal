...
- **Multi-action `CardAction` composition (see Variants)**: when `CardAction` holds more than one control, lay them out with `flex items-center gap-*` at the call site — `CardAction`'s own grid placement (top-right of `CardHeader`) is unchanged; only its *contents* compose more than one element. Not yet implemented in code (no consuming page exists for this composition yet) — documented here from Figma verification ahead of the "Our Marriage Champions" page being built, per the Component Creation Process's "document before standardizing."
...
- **Destination routes are placeholders** — only `/` (Home) and the two external URLs (`https://amfm.org/mmp`, `https://wedowedo.com`, taken directly from the Figma reference's own `href`/`target` attributes) are real. Every other internal `href` (`/dashboard`, `/marriage-champions`, the account menu's five links, etc.) is a placeholder path pending real routes — don't mistake these for a verified IA.
...
**Status**: Draft (documented from Figma audit — anatomy/content types verified against the source frame, but no code exists yet; row hover/focus, empty state, exact tokens, and responsive behavior are undesigned — see Implementation rules)
**Source**: `src/components/ui/table.tsx` (planned — not yet created)
...
Exact prop surface (e.g. whether `TableCell` takes a `variant` prop or is purely compositional per call site, matching `Card`'s sub-component precedent) is an implementation decision not yet made — no code exists for this component yet.

### Design tokens used

Not yet confirmed to pixel precision — this pass verified structure and column/row content types against Figma, not exact spacing/color values. Expected candidates to confirm against `DESIGN.md` before implementation (do not inline hex values): `border-border-secondary` (row/column dividers — the lighter divider tier already used for `/login`'s "or" rule, a plausible match for this table's hairlines), `text-text-secondary` (header cell labels), `text-foreground`/`text-muted-foreground` (cell text tiers, matching `ResourceListItem`'s existing title/description token split).
...
**Not evidenced.** The Figma reference is a fixed desktop composition (1272px table within a 1512px frame) with no mobile/tablet reference — the same category of gap already tracked for `HeartChartSummary`/`PricingCard`/`GlobalNav`. Whether narrow viewports get horizontal scroll or a card-per-row collapse is an open design decision; don't guess a mobile pattern without a Figma reference or explicit product direction.
...
- **Known gap**: the "Completed MMP" header cell's trailing info affordance implies a tooltip/definition interaction with no corresponding component documented in this file yet. Do not build a `Tooltip` speculatively from this single icon — confirm the interaction model (hover-only vs. keyboard-operable per `DESIGN.md`'s custom-widget accessibility standard) with design before adding one.
- This entry documents structure/anatomy validated against Figma; it does not yet have a code implementation. Build at `src/components/ui/table.tsx` (a visual-only primitive with no embedded business logic, matching `Input`/`Select`'s convention), following this project's shadcn/ui `new-york` conventions.

### Visual examples

Not yet rendered — no code exists yet. Add a `/design-system/components#table` entry once implemented.

---

## StatusTag

**Status**: Draft (documented from Figma audit — variants/colors verified against the source frame, but no code exists yet, and the underlying color tokens are not yet defined in `DESIGN.md` — see Implementation rules)
**Source**: `src/components/ui/status-tag.tsx` (planned — not yet created)
...
**None of the three variants match an existing `DESIGN.md` color token.** `status-success`/`status-warning`/`destructive` are single-shade tokens built for a different job (donut/bar fills, solid button backgrounds — see `HeartChartSummary`/`Button` above), not this bg/border/text pill system; reusing them here would visibly mismatch Figma. A new 3-tier badge token family (bg/border/text × success/error/warning) needs to be added to `DESIGN.md`'s Color tokens table before this component can be implemented without hardcoding hex values, per `CLAUDE.md`'s tokens-over-instances rule. Values to add (confirmed via Figma variable inspection, not yet present in `src/tokens/colors.css`):
...
- Verify each variant's text-on-background contrast meets WCAG AA once the token values above are formalized in `DESIGN.md` — not yet contrast-checked in this pass.
...
- **Consolidate with `HeartChartSummary`'s existing "Live Data" badge** (dot + label, currently a one-off inline implementation local to that component — see `HeartChartSummary`'s Anatomy above) rather than shipping a second, independently-styled pill primitive. Extract this as the shared primitive both consume once implemented.
- Do not implement until the token family above is added to `DESIGN.md` — do not inline the hex values at the call site.
- This entry documents structure/variants validated against Figma; it does not yet have a code implementation.

### Visual examples

Not yet rendered — no code exists yet. Add a `/design-system/components#statustag` entry once implemented.
