...
- **Dark mode: verified, with a known limitation.** `Card` itself (`bg-card`/`text-card-foreground`) correctly flips with `.dark` (confirmed by actually toggling it, per `IMPLEMENTATION.md`'s "No unverified dark mode" non-negotiable) — but this screen's field labels, description, borders, and `PricingCard` content all use the auth/onboarding-derived root-only tokens (`text-text-secondary`, `text-text-tertiary`, `border-border-brand`, `border-border-secondary`) documented as fixed-light in `DESIGN.md`'s "Auth/onboarding surfaces are theme-fixed" section. Confirmed by inspection: these render at their light-theme values against the now-dark card surface — legible (not a contrast failure) but not true dark-mode-adapted colors, since no dark-mode Figma reference exists for this frame (same category of gap as `HeartChartSummary`'s, see `DESIGN.md` Known gaps). This was already true of `Input`/`Label` on this screen before this change; not a regression introduced by the new fields, but now more visible given how much more of the screen those tokens cover. Revisit if/when a dark-mode Figma reference for `Onboarding/Create Profile` appears.
- ~~The `Onboarding/Create Profile` Figma frame specifies a title/description in a serif "Financier Display" font not yet defined in `DESIGN.md`~~ **Resolved**: `DESIGN.md`'s Typography system now defines `font-display`/`text-display-md` (Fraunces substituting for the licensed Financier Display face). `/create-profile`'s `CardTitle` renders `font-display text-display-md font-light text-foreground` (was `text-3xl font-semibold tracking-tight`); `CardDescription` renders `text-text-tertiary` (was the generic `text-muted-foreground`, a different gray — Figma's description color resolves to the `text-tertiary` token, `#535862`, exactly).
...
"Powered by" caption text (`text-xs`, `font-medium`, `tracking-[0.24px]`, `text-text-tertiary`) + adjacent logo mark (currently `font-display font-semibold text-foreground` styled text reading "amfm" — see Implementation rules).
...
`text-text-tertiary`, `text-xs` (caption); `font-display`, `text-foreground` (mark text approximation) — the real mark, once available, would be a raster/vector asset and not token-driven.

### Accessibility requirements

`role="img"` + `aria-label="AMFM — Association of Marriage & Family Ministries"` on the mark span, matching `HeartChartLogo`'s `alt="HeartChart"` precedent for naming the brand mark to assistive tech.
...
- **Asset still blocked, approximated instead of left unimplemented**: Figma only serves this asset via a temporary MCP asset URL, and this environment's outbound network policy blocks direct downloads from `www.figma.com` (confirmed via the agent proxy status endpoint — a `403`/policy denial, not a transient failure) — the same class of gap `DESIGN.md`'s "Known gaps" previously resolved for the HeartChart wordmark and `/login` photo by having the real export supplied directly, which hasn't happened yet here. Unlike those two (a detailed photo and a distinctive wordmark glyph), this mark is a short plain-word logotype ("amfm") low-fidelity-risk enough to approximate with styled text — same tier of approximation as `GoogleIcon` (see that entry) — rather than leave the "Free Membership" card's footer credit missing entirely. Status stays **Draft**, not Production Ready, specifically to flag this as not yet pixel-verified against the real mark.
- **Do not** attempt to redraw the real logo as SVG paths from the screenshot (that would misrepresent a hand-guess as a faithful reproduction) — the text approximation is deliberately plain, not a traced facsimile.
