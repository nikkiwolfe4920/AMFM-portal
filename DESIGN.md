...
| `brand-700` | `bg-brand-700` | First/lightest tier of `CourseCard`'s numbered step-header background scale — `#9f6348`, a darkened variant of `border-brand` (`#c07858`) at the same 44.483° hue angle. Figma's own Step 1 fill (`border-brand` verbatim) only clears 3.46:1 contrast with white text/icon — below WCAG AA's 4.5:1 for this non-"large text" label — so this token exists specifically to let Step 1 use white like Steps 2/3 instead of a higher-contrast but off-brand dark text color. Root-only, same class of gap as `highlight-gold`/`nav-*` — no Figma dark-mode reference exists. |
| `brand-900` | `bg-brand-900` | Third/darkest tier of `CourseCard`'s numbered step-header background scale — `#47261a`. Step 2 reuses `text-brand` (`#894e34`) by exact hex match; this is the one genuinely new tier at the *dark* end, since no existing token lands on this value (see `brand-700` above for the new tier at the light end). Root-only, same class of gap as `highlight-gold`/`nav-*` — no Figma dark-mode reference exists. |
...
- **`GlobalNav` isn't wired into the global app layout yet** — it is used by `MarriageChampionsPageShell` for `/marriage-champions` and `/marriage-champions-empty`, and demonstrated standalone at `/design-system/components#globalnav`, but it is not mounted in `src/app/layout.tsx` as a universal authenticated shell. Integrate it globally only once the broader dashboard IA exists, rather than forcing it onto today's public/onboarding routes, which are built on the fixed-light `PhotoBackdrop`/`AuthCard` pattern instead.
