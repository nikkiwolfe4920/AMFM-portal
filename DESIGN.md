...
| `wedo-brand` | `text-wedo-brand` | `WeDoCard`'s couple-count stat, pull-quote highlighted phrase, and decorative quotation mark — WeDo's own brand red, Figma `Colors/Red/500`, `#cd4745`. Distinct from `primary` (the app's own terracotta accent) — a different hue entirely, not a shade of the same color. Root-only, same class of gap as the other HeartChart Dashboard tokens above. |
...
- ~~`TopHero`'s background photo, `CourseCard`'s three per-step video thumbnails, and `FooterCta`'s background texture are all unavailable in this environment`~~ **Partially resolved**: `TopHero`'s congregation-stage photo and `CourseCard`'s three per-step video-cover photos were supplied directly and committed to `public/` (`Featured Training.png`, `Step-1.png`/`Step-2.png`/`Step-3.png`), the same "real export supplied directly" resolution path already used for the `/login` background photo — both now render via `next/image` (`fill`/`object-cover`, `unoptimized` since this repo has no `sharp` dependency) instead of the `nav-surface-from`→`nav-surface-to` placeholder gradient, with `TopHero` keeping Figma's left-to-right legibility scrim (bleeding to the card's true outer edge — see `COMPONENTS.md#tophero`) and `CourseCard` applying its own Figma-confirmed `bg-gradient-to-r from-black/80 to-transparent` scrim per step (see `COMPONENTS.md#coursecard`). `FooterCta`'s background texture remains unavailable — the Figma MCP's `download_assets`/raw-image URLs still all resolve to `www.figma.com`, which this environment's egress policy denies (confirmed via the agent proxy status endpoint — a `403` policy denial). It still renders a flat `bg-primary` instead of Figma's warm gradient/noise texture; replace once the real export is supplied and committed to `public/`, per the same precedent.
...
- **The HeartChart Dashboard data-viz palette (`chart-pie-*`/`chart-scale-blue-*`/`chart-participation-fill`) and `wedo-brand` are root-only** — no dark-mode Figma reference exists for the "HeartChart Dashboard / premium" frame (node `3727:29573`), same class of gap as `status-success`/`badge-*`/`nav-*` above. Add `.dark` values once a dark-mode Figma reference exists.
