...
| `wedo-brand` | `text-wedo-brand` | `WeDoCard`'s couple-count stat and pull-quote highlighted phrase — WeDo's own brand red, Figma `Colors/Red/500`, `#cd4745`. Distinct from `primary` (the app's own terracotta accent) — a different hue entirely, not a shade of the same color. Root-only, same class of gap as the other HeartChart Dashboard tokens above. |
