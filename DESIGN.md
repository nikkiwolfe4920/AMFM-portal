...
| `brand-900` | `bg-brand-900` | Third/darkest tier of `CourseCard`'s numbered step-header background scale — `#47261a`. Steps 1 and 2 reuse `border-brand` (`#c07858`) and `text-brand` (`#894e34`) by exact hex match; this is the one genuinely new tier, since no existing token lands on this value. Root-only, same class of gap as `highlight-gold`/`nav-*` — no Figma dark-mode reference exists. |
| `button-primary-icon` | `text-button-primary-icon` | `Button`'s `default` (primary) variant leading/trailing icon color — `#d89f88`, Figma's own `Component colors/Components/Buttons/button-primary-icon` variable, confirmed identical across every sampled `default`-variant icon instance ("Invite Marriage Champions", "Upgrade to Premium", "Get Started"). Deliberately distinct from `primary-foreground` (`#ffffff`, the same button's label color) — on `default`, the icon does not inherit the label's color via `currentColor` the way it does on every other variant; see `COMPONENTS.md#button` Implementation rules. Root-only, same class of gap as `brand-900`/`nav-*` — no Figma dark-mode reference exists. |
...
- **`button-primary-icon` is root-only**, same class of gap as `brand-900`/`badge-*` above — no Figma dark-mode reference exists yet for any of the sampled `default`-variant icon instances. Add a `.dark` value once one does.
- ~~`Button`'s `default`-variant icon was assumed to inherit the label's `text-primary-foreground` color via `currentColor`~~ **Resolved — was incorrect**: an earlier pass documented the "Invite Marriage Champions" icon (then node `3724:23184`) as confirmed white via `currentColor` inheritance, without actually inspecting Figma's underlying variables. Re-inspecting that node (now `4188:25971`) plus `4188:25952`/`4188:25956` ("Upgrade to Premium") and `1894:16263` ("Get Started") via `get_variable_defs` shows the icon fill is bound to a distinct `button-primary-icon` variable (`#d89f88`), never `text-white`. Fixed by adding the `button-primary-icon` token above and applying it in `Button`'s `default` variant (see `src/components/ui/button.tsx` and `COMPONENTS.md#button`) — every existing `default`-variant icon-leading button (`/marriage-champions-empty`, `/welcome`, `/heartchart-resources`) now renders the corrected color automatically, no call-site changes needed.
