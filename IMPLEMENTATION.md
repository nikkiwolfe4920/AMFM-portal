...
8. **Run the design-system governance check.** Before calling UI work complete, run `npm run check:design-system`. The default gate checks changed production source files for undocumented arbitrary visual values and raw hex values, and validates changed component files against `figma/component-map.json` / `COMPONENTS.md`.
...
- **No prose-only exceptions.** If an arbitrary visual value is truly unavoidable, it must be registered in `design-system/audits/exceptions.json` with the required metadata. Mentioning the value in `DESIGN.md`, `COMPONENTS.md`, a PR, or a comment is not enough for the checker to accept it.
...
4. If a truly unavoidable arbitrary visual value remains after those checks, register it in `design-system/audits/exceptions.json` with the full exception metadata and keep it visible in `npm run check:design-system` output. Treat this as a temporary or explicitly permanent exception, not a casual local override.
