...
`figma/component-map.json` is the canonical machine-readable mapping. For reusable components, it stores:

- `figmaComponent`: human-readable Figma summary for reviewers.
- `figmaSources`: structured source references with `fileKey`, colon-form `nodeId`, `name`, `url`, `role`, and `lastVerified`.
- `visualTargets`: optional ids from `design-system/audits/visual-comparisons/targets.json` that can regenerate a Figma/code review board for the component.
- `documentation`: the component's own `COMPONENTS.md` anchor.
- `implementation`: the owning source file.

The checker validates the implementation path, documentation anchor, structured Figma source shape, Figma URL/file/node consistency, visual-target array shape, and visual-target id existence in `design-system/audits/visual-comparisons/targets.json`. Generated screenshots stay under `design-system/audits/.generated/`; they are review evidence, not the source of truth.
...
|---|---|---|
| `Design System / Execution` (`3722:19910`) | July MVP working container | Top-level Figma source set for audit/reconciliation. Contains the current July MVP manifestations for onboarding screens (`3722:19915`, `3722:19911`, `3722:19912`), HeartChart Resources (`3722:19475`), the assigned modal family (`4207:25806`, `3724:20579`, `3724:23382`, `3727:32459`, `3727:32514`, `3727:32687`), settings modals (`3724:20992`, `3727:32947`), Marriage Champions states (`3724:23167`, `3724:23444`), and HeartChart dashboard states (`3899:27502`, `4255:30872`). Use this node first when comparing page/pattern/component manifestations against the current designer source. |
...
| `3724:20992` (`Settings/Church Profile` instance `3724:21598`) | Modal/Settings/Church Profile | `SettingsModalShell`, `SettingsSection`, `SettingsAssetUpload`, `SettingsCampusList`, `InputActionGroup`, and `ChurchProfileSettingsModal` — settings modal shell/navigation/form anatomy and Church Profile composed pattern (`COMPONENTS.md#churchprofilesettingsmodal`) |
| `4207:25806`, `1903:19737`, `3724:20579`, `3724:23382`, `3727:32459`, `3727:32514`, `3727:32687` | HeartChart modal family | `HeartChartModalShell` — shared modal overlay/header/body/footer shell for Modal / HeartChart link, Modal / invite user, Modal / quick tip, Modal / last 4 weeks, and HeartChart Resources / Quick Start (`COMPONENTS.md#heartchartmodalshell`) |
...
| `3727:25297` ("_Nav item base") | Expanded `NavItem` row with a "Coming Soon" badge (demoed on "Date Night Kits") | `GlobalNav`'s `comingSoon` item state and `NavComingSoonBadge` (`COMPONENTS.md#globalnav`) |
...
| `Our Marriage Champions / Empty` (`3724:23167`) | `/marriage-champions-empty` screen | `MarriageChampionsPageShell` (shared shell, page header), `BlurOverlay` (backdrop layer "image 54", node `3724:23178`, wrapping a decorative `Table`/`StatusTag` preview), `VideoPlayer` (node `3724:23180`, second confirmed use site), `Button size="sm"` (node `3724:23184`, "Invite Marriage Champions" — icon-leading composition, lucide's `FileBadge` matching Figma's `certificate-02`) — reusable `InviteUserModal` for "Modal / invite user" node (`3724:23382`) is implemented in this branch; route-level CTA wiring from this screen is outside this design-system pass — see `COMPONENTS.md` |
| `HeartChart Dashboard / premium` (`4255:30872`), `_Summary Data` region, right-hand instance | `/dashboard` hero row, WeDo card | `WeDoCard` — couple illustration exported directly to `public/We-do.png` and rendered responsively with `aspect-wedo-illustration`/`max-w-wedo-illustration`; `PointerCallout`'s `"left-diagonal"` tail exported to `public/speechbubblepointer.svg`; the pull-quote's decorative quotation mark is the verified large low-opacity display glyph treatment — see `COMPONENTS.md#wedocard` and `COMPONENTS.md#pointercallout` |
| `HeartChart Dashboard / premium` (`4255:30872`), `Scattergram` instance (`4255:30881`) | `/dashboard` relationship-health card | `CommitmentConnectionChart` uses the verified cropped scattergram asset (`public/relationship-health-scattergram.png`); `SnapshotVideoCard` uses the verified cropped video thumbnail (`public/relationship-health-snapshot-video.png`); parent row uses `grid-cols-dashboard-relationship-health` (695:564); the same instance's `Frame 477` also contains the `FullWidthBarChart`/`BarLineChart` instance at the card's bottom — see `COMPONENTS.md#commitmentconnectionchart`, `COMPONENTS.md#snapshotvideocard`, and `COMPONENTS.md#fullwidthbarchart` |
| "BarLineChart" master component (`1243:23077`) | `/dashboard` relationship-health card, bottom chart | `FullWidthBarChart` — confirmed via a direct Figma MCP pull (not screenshot-derived): top `yAxisTop` percentage scale, dashed `xLines` gridlines (solid boundary at 0%), per-bar `DataLabel` positioned at that bar's own value rather than a shared trailing column, and the two-stop bar-fill gradient — see `COMPONENTS.md#fullwidthbarchart` |
| "Data Dashboard Empty State" (`3899:27502`), instance "HeartChart Dashboard / No data" (`3727:29364`) | `/dashboard-empty` screen | `GlobalNav`, `FellowshipOfTheParksLogo`, `ElevatedCard`, `BlurOverlay` (wrapping the real `DashboardContent` composition instead of Figma's flattened backdrop image, node `2026:41512` — see `COMPONENTS.md#bluroverlay`), `Button size="compact"` (icon-leading "Share your Unique QR Code and Link" CTA, lucide's `QrCode` matching Figma's `qr-code-01`), `FooterCta` (shared instance of node `1909:25789`) — see `COMPONENTS.md` |
...
- `WeDoCard`'s pull-quote quotation mark — implemented as a large low-opacity display glyph (`&ldquo;`) rather than an exported vector asset, matching the verified dashboard frame treatment.
...
- If the component has a Figma source, add or update `figmaSources`; do not rely only on free-form prose in `figmaComponent`.
- If the component is part of visual validation, add or update `visualTargets` so future contributors can regenerate the comparison from repo-owned files.
