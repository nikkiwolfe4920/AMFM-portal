...
| `HeartChart Dashboard / premium` (`4255:30872`), `Scattergram` instance (`4255:30881`) | `/dashboard` relationship-health card | `CommitmentConnectionChart` uses the verified cropped scattergram asset (`public/relationship-health-scattergram.png`); `SnapshotVideoCard` uses the verified cropped video thumbnail (`public/relationship-health-snapshot-video.png`); parent row uses `grid-cols-dashboard-relationship-health` (695:564) — see `COMPONENTS.md#commitmentconnectionchart` and `COMPONENTS.md#snapshotvideocard` |
