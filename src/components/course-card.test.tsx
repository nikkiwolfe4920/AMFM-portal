...
describe("CourseCard", () => {
  it("uses a contrast-safe step-one header while preserving the Figma brand fill", () => {
    render(
      <CourseCard
        step={1}
    ...
    expect(header).toHaveClass("bg-border-brand", "text-foreground");
    expect(screen.getByText("STEP 1")).not.toHaveClass("text-white");
  });
